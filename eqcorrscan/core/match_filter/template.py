...
class Template(object):
    ...
    def _assign_trace_metadata(self):
        """
        ...
        event = self.event
        # Skip if it's an old tribe / template / event with no extra metadata
        if (not hasattr(event, 'extra') or
                not hasattr(event.extra, 'trace_ids')):
            return self
        ...
        for key, value in self.event.extra.items():
            ...
            if isinstance(value.value, str):
                try:
                    ...
                    if 'time' in key:
                        ...
                        conv_string = value.value.strip("[]")
                        if len(conv_string) > 0:  # otherwise eval fails
                            value.value = ast.literal_eval(conv_string)
        ...
        if n_traces < n_traces_metadata:
            Logger.info(
                'Need to split traces for template %s', self.name)
            ...
            msg = ('Template {0}: Read in more traces ({1}) than trace ' +
                    'metadata ({2}, event {3}), this should not happen.'
                        ...
                        self.name, n_traces, n_traces_metadata,
                        event.short_str())
            raise NotImplementedError(msg)
        ...
        for key, value in self.event.extra.items():
            ...
            for tr, tr_metadata_value in zip(self.st, value.value):
